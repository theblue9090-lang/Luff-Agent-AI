...
export default function StatsBar({ totalMcap, totalVolume, agentCount, holders }: StatsBarProps) {
  ...
  return (
    <section className="mx-auto -mt-2 max-w-7xl px-4 sm:px-6">
      <div className="grid grid-cols-2 gap-3 rounded-2xl border border-luff-border bg-white/[0.02] p-3 backdrop-blur-xl sm:grid-cols-4 sm:gap-4 sm:p-4">
        {stats.map((s) => (
          <div key={s.label} className="flex items-center gap-3 rounded-xl px-2 py-1.5">
